...
import one.tranic.sewlia.task.TPSBarTask;
...
public class TPSBarCommand extends Command {
    public TPSBarCommand() {
        super("tpsbar");
        this.setPermission("sewlia.command.tpsbar.self");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        if (args.length == 0) {
            ...
            if (!sender.hasPermission("sewlia.command.tpsbar.another")) {
        ...
        for (String target : args) {
            ...
            boolean self = (sender instanceof Player) && player != null && player.getUniqueId().equals(((Player) sender).getUniqueId());
    ...
    private Component updateTarget(Player player) {
        ...
        ((CraftPlayer) player).getHandle().tpsBar(result);

        return MiniMessage.miniMessage().deserialize("<green>Tpsbar toggled <onoff> for <target>",
    ...
    public @NotNull List<String> tabComplete(@NotNull CommandSender sender, @NotNull String alias, @NotNull String[] args) {
        if (args.length < 1 || !sender.hasPermission("sewlia.command.tpsbar.another")) return List.of();
